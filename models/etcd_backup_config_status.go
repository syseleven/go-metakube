...
func (m *EtcdBackupConfigStatus) validateConditions(formats strfmt.Registry) error {
	if typeutils.IsZero(m.Conditions) { // not required
		return nil
	}

	for i := 0; i < len(m.Conditions); i++ {
		if typeutils.IsZero(m.Conditions[i]) { // not required
			continue
		}
...
func (m *EtcdBackupConfigStatus) validateCurrentBackups(formats strfmt.Registry) error {
	if typeutils.IsZero(m.CurrentBackups) { // not required
		return nil
	}

	for i := 0; i < len(m.CurrentBackups); i++ {
		if typeutils.IsZero(m.CurrentBackups[i]) { // not required
			continue
		}
