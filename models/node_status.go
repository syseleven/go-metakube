...
func (m *NodeStatus) validateAddresses(formats strfmt.Registry) error {
	if typeutils.IsZero(m.Addresses) { // not required
		return nil
	}

	for i := 0; i < len(m.Addresses); i++ {
		if typeutils.IsZero(m.Addresses[i]) { // not required
			continue
		}
...
func (m *NodeStatus) validateAllocatable(formats strfmt.Registry) error {
	if typeutils.IsZero(m.Allocatable) { // not required
		return nil
	}
...
func (m *NodeStatus) validateCapacity(formats strfmt.Registry) error {
	if typeutils.IsZero(m.Capacity) { // not required
		return nil
	}
...
func (m *NodeStatus) validateNodeInfo(formats strfmt.Registry) error {
	if typeutils.IsZero(m.NodeInfo) { // not required
		return nil
	}
