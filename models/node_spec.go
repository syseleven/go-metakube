...
type NodeSpec struct {
	...
	// List of annotations to set on machine objects
	MachineAnnotations map[string]string `json:"machine_annotations,omitempty"`

	// List of annotations to set on nodes
...
func (m *NodeSpec) validateTaints(formats strfmt.Registry) error {
	if typeutils.IsZero(m.Taints) { // not required
		return nil
	}

	for i := 0; i < len(m.Taints); i++ {
		if typeutils.IsZero(m.Taints[i]) { // not required
			continue
		}
