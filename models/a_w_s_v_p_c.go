...
func (m *AWSVPC) validateCidrBlockAssociationSet(formats strfmt.Registry) error {
	if typeutils.IsZero(m.CidrBlockAssociationSet) { // not required
		return nil
	}

	for i := 0; i < len(m.CidrBlockAssociationSet); i++ {
		if typeutils.IsZero(m.CidrBlockAssociationSet[i]) { // not required
			continue
		}
...
func (m *AWSVPC) validateIpv6CidrBlockAssociationSet(formats strfmt.Registry) error {
	if typeutils.IsZero(m.Ipv6CidrBlockAssociationSet) { // not required
		return nil
	}

	for i := 0; i < len(m.Ipv6CidrBlockAssociationSet); i++ {
		if typeutils.IsZero(m.Ipv6CidrBlockAssociationSet[i]) { // not required
			continue
		}
